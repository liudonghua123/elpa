...
`read' input pattern."
  (let* ((input
          (unwind-protect (minibuffer-with-setup-hook #'el-search-read-pattern-setup-mb-hints
  ...
  (let (real-fun)
    (pcase regexps
      (`(:key ,specified-fun . ,more-regexps)
       (setq real-fun     (lambda (arg) (funcall specified-fun (funcall fun arg)))
...
function implicitly (but support to specify a :key nonetheless)."
  (declare (heuristic-matcher (apply-partially #'el-search--filename-matcher #'identity))
  ...
  (let ((file-name-matcher (apply #'el-search--filename-matcher #'identity regexps)))
...
(el-search-defpattern file (&rest regexps)
  ...
  (declare (heuristic-matcher (apply-partially #'el-search--filename-matcher #'file-name-nondirectory))
           (inverse-heuristic-matcher t))
  (el-search-defpattern--check-args "file"
                                    (if (eq (car-safe regexps) :key) (cddr regexps) regexps)
                                    #'el-search-regexp-like-p)
  (let ((file-name-matcher (apply #'el-search--filename-matcher #'file-name-nondirectory regexps)))
    `(guard (funcall ',file-name-matcher (current-buffer) nil))))
...
(el-search-defpattern dir (&rest regexps)
  ...
  (el-search-defpattern--check-args "dir"
                                    (if (eq (car-safe regexps) :key) (cddr regexps) regexps)
                                    #'el-search-regexp-like-p)
  (let ((file-name-matcher (apply #'el-search--filename-matcher #'el-search--file-directory regexps)))
    `(guard (funcall ',file-name-matcher (current-buffer) nil))))
...
(defun el-search-occur--next-match (&optional backward)
  (let ((pos (point)) new-pos)
    (cl-flet ((done (pos) (when-let ((match-nbr (get-char-property pos el-search-match-prop)))
                            (and (not (= (point) (if backward (point-min) (point-max))))
                                 (not (eq match-nbr
                                          (get-char-property (1- pos) el-search-match-prop)))))))
                              ...
                              (cl-flet ((insert-match-and-advance
                                         ...
                                         (let ((insertion-point (point)))
                                           ...
                                           (let ((ov (make-overlay insertion-point (point) nil t)))
                                             ...
                                             ;; FIXME: I guess we don't need both of these
                                             (overlay-put
                                              ov 'el-search-match (list (or file buffer) match-beg))
                                             (overlay-put
                                              ov el-search-match-prop `(,buffer ,match-beg ,file ,nbr)))
                                            ...
                                            (`((,_ ,(and (pred (> working-position)) mb) ,_) ,nbr ,_)
                                             (let ((ov-start (+ main-match-beg (- mb match-beg))))
                                               (overlay-put
                                                ...
                                                el-search-match-prop `(,buffer ,mb ,file ,nbr)))
                                             (stream-pop matches)
                                             t)))))
                                  (insert
                                   (with-current-buffer buffer
...
(defun el-search-query-replace-ediff-regions ()
  ...
  (let* ((buffer-orig (generate-new-buffer "El-search Orig"))
         (buffer-b (ediff-make-cloned-buffer (current-buffer) "El-search Replacement"))
         (delete-temp-buffers
    ...
    (apply #'ediff-regions-internal
           (nconc
            ...
            (list (list (lambda () (add-hook 'ediff-quit-hook delete-temp-buffers t t)))
...
;; be prompted whether to use the edited replacement expression."
                                     ...
                                     (use-local-map
                                      (let ((map (make-sparse-keymap)))
                                        ...
                                        map))
                                     (let ((el-search-query-replace--matched-sexp
                                            original-text))
                                       (recursive-edit))
                                     (let ((content-now
                                            (with-current-buffer buffer
                                       ...
                                       (when (and (not (string= to-insert content-now))
                                                  (y-or-n-p "Use modified buffer content?"))
                                         ...
                                         (list
                                          ...
                                          `(?r "r"
                                               ,(if (not replaced-this)
                                                    "Replace this match but don't move"
                                                  "Restore match"))
                           ...
                           (or el-search--initial-mb-contents
                               (and (or (eq last-command 'el-search-pattern)
                                    ...
                                    (if (equal (el-search-read (car el-search-pattern-history))
                                      ...
                                      (car el-search-pattern-history))))))
                      (minibuffer-with-setup-hook #'el-search-read-pattern-setup-mb-hints
